 /*! laydate-v5.0.1 日期与时间组件 MIT License  http://www.layui.com/laydate/  By 贤心 */ ;
! function() {
	"use strict";
		...
		t = {
			getPath: function() {
				var e = document.scripts,
					t = e[e.length - 2],
					n = t.src;
				// 获取 时间插件 laydate.js 路径
  				for(var i=0;i<e.length;i++){
  					if(e[i].src.indexOf("laydate.js")>-1){
  						n = e[i].src;
  						t = e[i];
  					}	
  				}
	...
	w.prototype = [], w.prototype.constructor = w, T.extend = function() {
		...
		if (w[0] < 1 && (w[0] = 1), "year" === e) {
			...
			b < 1 && (b = M = 1), T.each(new Array(15), function(e) {
				...
				M == w[0] && T(a).addClass(o), a.innerHTML = M + D, d.appendChild(a), n.limit(T(a), {
					year: M
				}, t), M++
		...
		if (y && h.removeChild(y), h.appendChild(d), "year" === e || "month" === e) T(n.elemMain[t]).addClass("laydate-ym-show"), T(d).find("li").on("click", function() {
			...
			if (!T(this).hasClass(s)) {
				...
				T(n.footer).find(p).removeClass(s); // 用于最大值/最小值 选中 确认按钮 可以点击（清除禁止点击事件） 
	...
	}, D.prototype.setValue = function(e) { // 选择时间触发
		var t = this,
			n = t.config,
			a = t.bindElem || n.elem[0],
			i = t.isInput(a) ? "val" : "html";
		return "static" === n.position || T(a)[i](e || ""), this
	}, D.prototype.stateDate = function() {  // 区间范围 限制 两个input 框
		var t = this,
			n = t.config,
			start = true,
			current_time = document.getElementById(n.sectionId).value,
			stop_time = n.elem[0].value;
		// 比较时间大小
		...
		if(current_time && current_time!=""){
			switch (n.dateState) {
				case "startDate":
					...
					break;
				case "endDate":
					if(CompareDate(stop_time,current_time)){
						start = false;
					}
					break;
			}
		...
		}), T(e.footer).find("span").on("click", function() { // 确定按钮 事件 
...
/*  
    ...
    时间范围 开始与结束 区间选择方法 
      var start = {
            ...
            max: $('#endDate').val(),
            show: true, //直,接显示
            closeStop: '#startDate', //这里代表的意思是：点击 test1 所在元素阻止关闭事件冒泡。如果不设定，则无法弹出控件
            btns: ['now', 'confirm'],
            done:function(value,date,endDate){//改变日期触发事件
                var code=$(".center_state li").last().attr("data-code");
        ...
        var end = {
            ...
            btns: ['now', 'confirm'],
            show: true, //直接显示
            closeStop: '#endDate', //这里代表的意思是：点击 test1 所在元素阻止关闭事件冒泡。如果不设定，则无法弹出控件
        ...
        $('#startDate').on('click', function(e){ //假设 test1 是一个按钮
            ...
            myFrame.laydate(start);
        });
        $('#endDate').on('click', function(e){ //假设 test1 是一个按钮
            end.min = $('#startDate').val();
            myFrame.laydate(end);
        });

...
(function (window, undefined) {
    ...
    * */
    myFrame.checkType = function (str, type) {
        switch (type) {
            case 'email': //邮箱
                return /^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/.test(str) || /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
            case 'phone':  //手机
...
//表单检验
   ...
   *
   * */
    myFrame.replaceClass = function (obj, newName, oldName) {
  ...
  * */
    ...
    //添加类名
    myFrame.addClass = function (obj, classStr) {
        if (!this.hasClass(obj, classStr)) {
            obj.className += " " + classStr;
        }
    };
    //删除类名
    myFrame.removeClass = function (obj, classStr) {
        if (this.hasClass(obj, classStr)) {
    ...
    /*
    * n 代表金额
    * 使用:myFrame.upDigit(168752632);   "人民币壹亿陆仟捌佰柒拾伍万贰仟陆佰叁拾贰元整"
    *     myFrame.upDigit(-1693);   "欠人民币壹仟陆佰玖拾叁元整"
    * */
    myFrame.upDigit = function (n) {
        var fraction = ['角', '分', '厘'];

$(function(){
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			$("body").append("<div id='bigAutocompleteContent' class='bigautocomplete-layout'></div>");//添加div元素
			$(document).bind('mousedown',function(event){//bind() 方法为被选元素添加一个或多个事件处理程序，并规定事件发生时运行的函数。
				var $target = $(event.target);//触发该事件的 DOM 元素
		...
		this.autocomplete = function(param){
			...
			if($("#tianJia").length > 0 && $("#bigAutocompleteContent").length <= 0){
			...
			this.config = {
			               ...
			               data:[{title:"（控告）破坏社会主义市场经济秩序罪"},
							   ...
							   {title:"侵犯公民人身权利"},
							   {title:"民主权利罪"},
							   {title:"破坏社会主义市场经济秩序罪"},{title:"军人违反职责罪"},
							   {title:"危害国防利益罪"},
							   {title:"贪污贿赂罪"},
							   {title:"渎职罪"},
							   {title:"贪污贿赂罪"},
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					setPositionAutocompleteContent( config.width ); //新加的
			...
			function setPositionAutocompleteContent( width ){ //新增加的
				var offset = $this.offset();
				var top=offset.top;
				$bigAutocompleteContent.width(width);
			...
			$this.focus(function(){
				...
				//实现 focus就显示下拉框的代码
				var config = $this.data('config');
				setPositionAutocompleteContent( config.width );
